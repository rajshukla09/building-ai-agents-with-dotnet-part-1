...
public sealed class ToolExecutionPipelineTests
{
    ...
    public async Task TransientFailure_RetriesThenRecordsSuccess()
    {
        ...
        object? result = await pipeline.ExecuteWithPolicyAsync("TestTool", new
        {
            value = 1
        },
    ...
    public async Task ExhaustedRetries_RecordFailureAndReason()
    {
        ...
        await Assert.ThrowsAsync<ToolExecutionFailedException>(() => pipeline.ExecuteWithPolicyAsync(
            "TestTool", new
            {
            }, ToolInvocationMode.ModelSelected, null,
    ...
    public async Task Timeout_IsRecordedAndNotRetried()
    {
        ...
        await Assert.ThrowsAsync<ToolExecutionFailedException>(() => pipeline.ExecuteWithPolicyAsync(
            "SlowTool", new
            {
            }, ToolInvocationMode.Deterministic, null,
    ...
    public async Task Trace_PreservesExecutionOrderAndInvocationModes()
    {
        ...
        using ExecutionTraceScope scope = recorder.BeginRequest();
        await pipeline.ExecuteWithPolicyAsync("First", new
        {
        }, ToolInvocationMode.Deterministic, 1,
            _ => Task.FromResult<object?>(1), CancellationToken.None);
        await pipeline.ExecuteWithPolicyAsync("Second", new
        {
        }, ToolInvocationMode.ModelSelected, 2,
    ...
    public void Trace_CapturesClassificationTelemetry()
    {
        ...
        {
            Intent = SmartTravelPlanner.Contracts.RequestIntent.DistanceLookup,
            Steps = [new() { Order = 1, Tool = SmartTravelPlanner.Api.Classification.ToolType.Distance }]
        };
        ExecutionPlanValidationResult validation = new()
        {
            IsValid = true,
            Errors = []
        };
        DateTimeOffset classificationCompletedAt = clock.GetUtcNow();
        recorder.RecordExecutionPlanClassification(startedAt, classificationCompletedAt, plan);
        DateTimeOffset validationStartedAt = clock.GetUtcNow();
        DateTimeOffset validationCompletedAt = clock.GetUtcNow();
        recorder.RecordExecutionPlanValidation(validationStartedAt, validationCompletedAt,
            validation, false, null, null, null, validation);
        recorder.RecordToolExecutionDuration(12);
        ...
        Assert.Equal(startedAt, executionPlan.ClassificationStartedAt);
        Assert.Equal(classificationCompletedAt, executionPlan.ClassificationCompletedAt);
        Assert.Equal(validationStartedAt, executionPlan.ValidationStartedAt);
        Assert.Equal(validationCompletedAt, executionPlan.ValidationCompletedAt);
        ...
        Assert.Equal(12, executionPlan.ToolExecutionDurationMs);
    }
