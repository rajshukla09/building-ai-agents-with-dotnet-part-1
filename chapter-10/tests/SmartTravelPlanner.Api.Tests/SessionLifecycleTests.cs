...
public sealed class SessionLifecycleTests
{
    ...
    private sealed class TestContext
    {
        ...
        public InMemoryConversationStore Store
        {
            get;
        }
    ...
    private sealed class FakeTravelAgent : ITravelAgent
    {
        ...
        public Task<AgentResult<TripPlan>> ExecuteAsync(
            TravelAgentRequest request,
            CancellationToken cancellationToken = default) =>
            Task.FromResult(AgentResult<TripPlan>.Success(CreatePlan(request.TravelRequest.Destination), Metadata()));

        private static AgentExecutionMetadata Metadata()
        {
            DateTimeOffset now = DateTimeOffset.UtcNow;
            return new(nameof(FakeTravelAgent), nameof(TripPlan), AgentExecutionStatus.Succeeded, 1, true, false, false, false, false, now, now, 0, []);
        }
