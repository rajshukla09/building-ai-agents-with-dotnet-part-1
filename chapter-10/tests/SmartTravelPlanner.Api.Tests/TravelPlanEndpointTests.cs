...
public sealed class TravelPlanEndpointTests
{
    ...
    public async Task InvalidRequestReturnsBadRequest(string destination, int durationDays)
    {
        ...
        HttpResponseMessage response =
            await client.PostAsJsonAsync("/api/travel/plan", new TravelPlanRequest(destination, durationDays));

        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        Assert.Equal(0, factory.Workflow.CallCount);
    }
    ...
    public async Task PreferencesOverMaximumLengthReturnsBadRequest()
    {
        ...
        HttpResponseMessage response =
            await client.PostAsJsonAsync("/api/travel/plan", new TravelPlanRequest("Jaipur", 2, new string('a', 501)));
    ...
    public async Task ValidRequestReturnsStructuredTripPlan()
    {
        ...
        HttpResponseMessage response =
            await client.PostAsJsonAsync("/api/travel/plan", new TravelPlanRequest("Jaipur", 2, "Local food"));
        ...
        Assert.Equal("Jaipur", factory.Workflow.LastRequest?.Destination);
        Assert.Equal(1, factory.Workflow.CallCount);
    }

    [Fact]
    public async Task WorkflowFailureReturnsControlledServerError()
    {
        using TestApplicationFactory factory = new(CreatePlan(), new InvalidOperationException("provider details"));
        using HttpClient client = factory.CreateClient();

        HttpResponseMessage response =
            await client.PostAsJsonAsync("/api/travel/plan", new TravelPlanRequest("Jaipur", 2));
    ...
    private static TripPlan
    CreatePlan() => new() { Destination = "Jaipur", DurationDays = 2, Summary = "A balanced introduction to Jaipur.",
                            Days = [CreateDay(1, "Historic Jaipur"), CreateDay(2, "Culture and markets")] };

    private static TripDay CreateDay(int number, string title) => new() {
        DayNumber = number, Title = title,
        Activities = [new TripActivity { Time = "09:00", Name = "Guided walk", Description = "Explore local landmarks.",
                                         Category = "Sightseeing", Notes = "Arrive early." }]
    };

    private sealed class TestApplicationFactory : WebApplicationFactory<Program>
    {
        public TestApplicationFactory(TripPlan response,
                                      Exception? exception = null) => Workflow = new FakeWorkflowService(response,
                                                                                                         exception);

        public FakeWorkflowService Workflow { get; }
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureServices(services =>
                                      {
                                          services.RemoveAll<ITravelWorkflowService>();
                                          services.AddSingleton<ITravelWorkflowService>(Workflow);
                                      });
        }
    }

    public sealed class FakeWorkflowService(TripPlan response, Exception? exception) : ITravelWorkflowService
    {
        public int CallCount { get; private set; }

        public TravelPlanRequest? LastRequest { get; private set; }

        public Task<TripPlanResponse> ExecuteAsync(TravelPlanRequest request,
                                                   CancellationToken cancellationToken = default)
        {
            CallCount++;
            LastRequest = request;
            return exception is null ? Task.FromResult(CreateResponse(response))
                                     : Task.FromException<TripPlanResponse>(exception);
        }

        public Task<TripPlanResponse>
        ExecuteExistingRunAsync(Guid runId, TravelPlanRequest request, string originalRequest,
                                CancellationToken cancellationToken = default) => ExecuteAsync(request,
                                                                                               cancellationToken);

        private static TripPlanResponse CreateResponse(TripPlan plan)
        {
            DateTimeOffset now = DateTimeOffset.UtcNow;
            return new(plan, new ExecutionTrace { StartedAt = now, CompletedAt = now, TotalDurationMs = 0 });
        }
    }
}
