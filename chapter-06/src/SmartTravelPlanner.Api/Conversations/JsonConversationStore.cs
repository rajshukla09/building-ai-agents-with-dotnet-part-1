...
public sealed class JsonConversationStore : IConversationStore
{
    ...
    public ConversationState Add(AgentSession session)
    {
        ...
        do
        {
            conversation = new ConversationState(Guid.NewGuid(), session, _timeProvider.GetUtcNow(), _options.ExpirationTimeout);
        }
        ...
        try
        {
            ...
            foreach (JsonElement element in conversations.EnumerateArray())
            {
                try
                {
                    ...
                    ConversationState state = new(document.Id, session, document.CreatedAt, document.LastActivityAt,
    ...
    private ConversationDocument ToDocument(ConversationState conversation)
    {
        ...
        return new ConversationDocument(metadata.ConversationId, _sessionSerializer.SerializeSession(conversation.Session),
