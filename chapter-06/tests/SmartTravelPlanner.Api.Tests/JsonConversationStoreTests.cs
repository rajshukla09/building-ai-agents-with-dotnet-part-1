...
public sealed class JsonConversationStoreTests : IDisposable
{
    ...
    {
        public JsonElement SerializeSession(AgentSession session) => JsonSerializer.SerializeToElement(new { kind = "test" });
    ...
    private sealed class UnusedTravelAgent : ITravelAgent
    {
        public Task<TripPlanResponse> CreateItineraryAsync(
            SmartTravelPlanner.Api.Contracts.TravelPlanRequest request,
