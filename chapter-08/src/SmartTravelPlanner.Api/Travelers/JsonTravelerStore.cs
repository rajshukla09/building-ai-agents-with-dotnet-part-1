...
public sealed class JsonTravelerStore : ITravelerStore
{
    ...
    public TravelerProfile Add()
    {
        ...
        do profile = new TravelerProfile { TravelerId = Guid.NewGuid(), CreatedAt = _timeProvider.GetUtcNow() };
    ...
    private void Load()
    {
        ...
        try
        {
            ...
            foreach (TravelerProfile profile in document?.Travelers ?? [])
                if (profile.TravelerId != Guid.Empty && profile.CreatedAt != default) _travelers[profile.TravelerId] = profile;
        }
