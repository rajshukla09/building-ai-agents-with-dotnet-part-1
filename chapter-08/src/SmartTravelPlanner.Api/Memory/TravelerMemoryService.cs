...
public sealed class TravelerMemoryService(ITravelerMemoryStore store, TimeProvider timeProvider)
{
    ...
    public TravelerMemory Save(Guid travelerId, TravelerMemoryRequest request)
    {
        ...
        FoodPreferences = Clean(request.FoodPreferences), ActivityInterests = Clean(request.ActivityInterests),
        TravelPace = Clean(request.TravelPace), BudgetPreference = Clean(request.BudgetPreference),
        AccessibilityRequirements = Clean(request.AccessibilityRequirements), UpdatedAt = timeProvider.GetUtcNow()
        });
    }
    public bool Delete(Guid travelerId) => store.Delete(travelerId);
    public string BuildContext(Guid travelerId)
    {
        TravelerMemory? memory = Get(travelerId); if (memory is null) return string.Empty;
        return $"""
