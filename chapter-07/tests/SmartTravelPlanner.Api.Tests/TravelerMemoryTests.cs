...
public sealed class TravelerMemoryTests : IDisposable
{
    ...
    public async Task NewConversationUsesMemoryWhileConversationStateRemainsSeparate()
    {
        ...
        Assert.Contains("vegetarian", agent.LastMessage);
        Assert.Contains("relaxed", agent.LastMessage);
    ...
    private sealed class FakeTravelAgent : ITravelAgent
    {
        ...
        public Task<TripPlanResponse> SendMessageAsync(string message, AgentSession session,
        ...
        CreateItineraryAsync(TravelPlanRequest request,
